...
const CERT_REQUIRED = new Set(['name'])
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {fields.map((k) => (
            <AdminField
              key={k}
              label={k}
              required={CERT_REQUIRED.has(k)}
              className={['criteria'].includes(k) ? 'sm:col-span-2' : ''}
            >
          ...
          <AdminField label="courses (JSON array)" className="sm:col-span-2">
            <textarea value={form.courses} onChange={(e) => setForm((f) => ({ ...f, courses: e.target.value }))} rows={3} className="w-full rounded-xl border px-3 py-2 text-sm font-mono" />
          </AdminField>
          <AdminField label="benefits (JSON array)" className="sm:col-span-2">
            <textarea value={form.benefits} onChange={(e) => setForm((f) => ({ ...f, benefits: e.target.value }))} rows={3} className="w-full rounded-xl border px-3 py-2 text-sm font-mono" />
          </AdminField>
        </div>
