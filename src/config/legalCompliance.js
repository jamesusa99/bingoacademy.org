/** Footer compliance badges and privacy policy copy for K-12 / overseas procurement. */

export const LEGAL_COMPLIANCE = {
  siteName: 'Bingo Academy',
  contactEmail: 'privacy@bingoacademy.org',
  badges: [
    {
      id: 'coppa',
      label: 'COPPA Compliant',
      short: '100% student privacy by design',
      icon: '🛡️',
    },
    {
      id: 'gdpr',
      label: 'GDPR Ready',
      short: 'EU data protection (GDPR)',
      icon: '🇪🇺',
    },
    {
      id: 'privacy',
      label: 'Privacy First',
      short: 'Minimal data collection',
      icon: '🔒',
    },
  ],
  footerTagline: 'Student privacy by design · COPPA-aware · GDPR-ready practices',
}
